...
from train import Generator, check_environment


def parse_args():
    parser = argparse.ArgumentParser(
        description="Fundus-GAN WGAN-GP: Generate fundus images from trained model",
    ...
    parser.add_argument("--grid_size", type=int, default=10, help="网格大小 (0=单张)")
...
def main():
    ...
    for idx in range(min(args.num_images, len(all_images))):
        save_image(all_images[idx], os.path.join(singles_dir, f"sample_{idx:04d}.png"), normalize=False)
