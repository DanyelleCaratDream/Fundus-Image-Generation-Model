...
H1("2. 评估指标与评分标准")
H2("2.1 为什么用同一套评估体系")
P(f"Phase A/B 已经为 6 个深度方法建好了一套评估体系（通用层 + 专用层 + 六维门控综合评分）。传统 ML 方法复用同一套，才能和深度方法在同一个标尺上横向对比——这正是\"补丁拼接 {score_of('patch'):.1f} 分 vs FiLM 扩散 {score_of('film_l1lpips'):.1f} 分\"这类结论的前提。否则各说各话，报告没有说服力。")
...
P("本报告早期按\"传统 ML 无条件图跳过 C2ST\"执行，结果补丁拼接打出 62.9 分的伪高分——逼近当年深度最佳 70.9（min-max 相对分，模型库后续扩大已整体平移，现版各分数见第 8 章）。排查发现根因是评分口径不均：深度模型的 D2 维度含 C2ST（权重 0.40），而 ML 基线没有，导致 D2 在不同指标集上对比。补跑 C2ST 后补丁大幅回落、GMM/PCA 同步重估，全部同口径。这条教训已写进执行计划：C2ST 必跑，不能跳过。")
...
H2("6.4 结果（含门控发现）")
P(f"总分 {score_of('patch'):.1f} 分（完成时 42.1），FID 159.2（三个 ML 里最低，甚至比部分深度模型还低），1-NN=0.995。病灶保留率最接近 1（出血 0.94、渗出 1.16），血管 Wasserstein 最小——因为像素本来就是真的。C2ST=1.000：真实像素拼接被小 CNN 一眼识破。记忆检测通过（接近复制占比 0）。人眼评分【待人工评估】。")
...
P("三个基线的结论指向同一个方向：单一传统 ML 不行，但\"组合\"可行。文献里最有支撑的两条组合（均有眼底专版验证）已按六段式跑完：泊松病变重排（EX-004）与 Retinex 光照交换（EX-005）。它们都产出了三个基线做不到的东西——保留真实纹理的同时引入新变化，真正\"相似但不相同\"。")

H2("7.1 泊松病变重排（EX-004，80.8 分）")
H3("7.1.1 是什么")
P("330 张全是重度 DR = 一座病变模板库。从图 A 抠一簇渗出、图 B 抠一块出血，用泊松编辑（cv2.seamlessClone NORMAL_CLONE）无缝贴到宿主图 D 上，再加全局旋转/色彩微变换，产出\"新病变组合\"的严重 DR 图。它不需要知道\"哪块是病灶\"——随机椭圆贴片簇（r60-150px @512）即可搬运病灶样内容。", "它")
H3("7.1.2 怎么做（略写）")
CODE("python scripts/poisson_gen.py --num_images 300 --seed 42 --max_patches 10 --patch_radius 100 --rot 15 --color_jit 0.18")
H3("7.1.3 为什么有用")
P("泊松编辑只保留插入区域的梯度、用宿主图边界条件求解，贴上去的病灶无缝融合、不破坏宿主图。它生成的是经典增广永远做不到的东西——新的病灶组合。（Yu 2021 BOE：DR 筛查优于过采样/裁剪/旋转）")
H3("7.1.4 结果")
P("总分 80.8（完成时 87.5；min-max 相对分随模型库扩大平移，下同），全场第 1。六维 D1 0.97 / D2 0.87 / D3 0.91 / D4 1.00 / D5 1.00 / D6 0.00。复制率 0.3%。FID 58.1（深度最佳 178.8）｜ C2ST 0.804（全场第 2 低）｜ BRISQUE 3.60≈真实 3.69 ｜ 病灶保留 出血 1.10/渗出 0.97 ｜ 血管 0.070 vs 真实 0.077。人眼评分：用户确认\"几乎看不出来是生成的\"。")
sixdim_table("泊松病变重排(EX-004)", "poisson")
IMG(os.path.join(EVAL, "poisson", "singles", "sample_0000.png"), 2.2, "泊松病变重排示例 sample_0000（真实病灶被搬运重组，血管/布局继承宿主图）")
H3("7.1.5 致命缺点")
...
H3("7.1.6 下一步")
P("泊松验证了\"新病灶组合\"假设成立：重排比纯拼接多一层语义（保留宿主图结构 + 搬运真实病灶）。它进入 Phase D 下游分类器验证候选——重点检验生成的重度图能否提升分类器对重度类的 Recall。")

H2("7.2 Retinex 光照交换（EX-005，72.5 分）")
H3("7.2.1 是什么")
P("把每张图分解为光照 L × 反射 R（经典单尺度 Retinex，log 域高斯低通分解），跨图交换光照：新图 = R_A · L_A^(1-α) · L_B^α（α∈[0.6,0.9] 每张随机）。反射 R 承载解剖结构（血管/视盘/病灶），光照 L 承载采集明暗色温。交换 L 只改\"看起来的调子\"，诊断语义几乎不变。", "它")
H3("7.2.2 怎么做（略写）")
CODE("python scripts/retinex_gen.py --num_images 60 --seed 42 --kernel 41 --alpha_lo 0.6 --alpha_hi 0.9 --mem_ref ../../eval_data/real --mem_thr 0.85 --max_retry 20\n# Retinex 分解 → 跨图光照 α 插值 → 重组 → 近复制过滤（NN-SSIM>0.85 重采样，方案 3）")
H3("7.2.3 为什么有用")
P("反射 R 保留结构语义，光照 L 交换改变全局明暗/色温，两者解耦让\"结构不变 + 风格变\"成为可能——这是深度无条件生成器难以显式控制的光照维度。（Zhang 2022 CBM：分割提升 9.6% Dice）")
H3("7.2.4 结果")
P("总分 72.5，全场第 2。六维 D1 0.94 / D2 0.82 / D3 0.72 / D4 0.92 / D5 0.92 / D6 0.10。复制率 0%（方案 3 过滤后归零）。C2ST AUC 0.744——全场最佳，最难被识破｜ FID 117.9 / KID 0.056（全场最低，底图复用所致）｜ BRISQUE 3.70≈真实 3.69。病灶保留 出血 1.06/渗出 0.44（渗出偏低）。亮度中位 77（真实 61，部分图偏暗）。人眼评分【待人工评估】。")
sixdim_table("Retinex光照交换(EX-005)", "retinex")
IMG(os.path.join(EVAL, "retinex", "singles", "sample_0000.png"), 2.2, "Retinex 光照交换示例 sample_0000（结构继承底图，光照来自供体图）")
H3("7.2.5 致命缺点")
BULLET("不生成新病灶形态：结构完全继承底图，病灶组合/布局不变")
BULLET("平均 NN-SSIM 0.439 偏高 → D6 记忆 0.10 弱项（光照交换天然保留底图结构）")
BULLET("亮度受供体光照影响：部分图偏暗/偏亮")
H3("7.2.6 下一步")
P("Retinex 验证了\"安全多样化\"假设：抗识破全场最佳 + 复制归零，适合作为\"相似但不相同\"的多样性放大器。同样进入 Phase D 下游验证候选。")
...
H2("7.4 为什么这两个组合值得做")
P("深度无条件生成器难以显式控制\"病灶种类/密度/位置\"与光照的组合；而泊松/Retinex 用显式规则做到——泊松造新病灶组合、Retinex 换光照风格。这正是本报告的核心论点：传统 ML 组合做深度方法做不到的事，作为数据扩充具有补充价值。")
...
H1("8. 结论")
H2("8.1 项目回顾")
P(f"本报告用 330 张严重 DR 眼底彩照，系统试了三类传统 ML 基线（PCA {score_of('pca'):.1f} / GMM {score_of('gmm'):.1f} / 补丁 {score_of('patch'):.1f}）与两条组合管线（泊松病变重排 {score_of('poisson'):.1f} / Retinex 光照交换 {score_of('retinex'):.1f}），与 Phase A/B 的 6 个深度方法在同一套评估体系下横向对比（深度最佳 FiLM+LPIPS 当前 {score_of('film_l1lpips'):.1f}）。⚠️ 综合评分是 min-max 相对分数，会随模型库扩大而平移，本报告各表使用同一版 _scores.json。")

IMG(os.path.join(ROOT, "research-report", "figures", "score_overview.png"), 6.0, "图 2：11 个模型综合评分总览（六维门控 0-100，min-max 相对分）")

H2("8.2 核心发现")
BULLET(f"纯传统 ML 直接生成不可行：PCA/GMM 的过平滑证实\"参数化方法必败于非高斯+强结构\"；补丁虽得 {score_of('patch'):.1f}，但那是真实像素的平凡胜利，不是生成。")
BULLET("评分体系盲区被暴露：补丁伪高分（62.9→补跑 C2ST 后大幅回落）证明\"用真实像素的方法必然在病灶/血管/颜色维度高分\"，跨\"真实重排 vs 生成\"对比时 C2ST 必须跑。")
BULLET("非参数保\"相似\"、参数化保不了：补丁法保纹理真实但缺结构，恰好引出组合管线。")
BULLET("组合管线让\"相似但不相同\"落地：泊松病变重排（80.8）与 Retinex 光照交换（72.5）都大幅超过三个基线，且复制率可控（0.3% / 0%）——泊松造新病灶组合、Retinex 换光照风格，做的是深度无条件生成器难以显式控制的事。")
